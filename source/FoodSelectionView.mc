...
//! Level 1 of food selection: subcategories or brands depending on filter order.
//! Each row is split: left zone navigates deeper, right zone (80px) toggles all in scope.
class FoodSelectionView extends WatchUi.View {
    ...
    //                    "toggleAll" => Boolean, "modeToggle" => Boolean, "toggleX" => Number }
    ...
    function onUpdate(dc as Dc) as Void {
        ...
        // Mode toggle row: explicit button showing current navigation order
        var modeRowHeight = 38;
        drawModeToggleRow(dc, yPos, width, modeRowHeight);
        rowCoordinates.add({ "label" => "", "y0" => yPos, "y1" => yPos + modeRowHeight,
                             "toggleAll" => false, "modeToggle" => true, "toggleX" => 0 });
        yPos += modeRowHeight;

        // Toggle-all row (full-row toggle, scope = all items)
        var rowHeight = 54;
        var scopeSelected = appState.countSelected(allItems);
        var scopeTotal = allItems.size();
        drawToggleAllRow(dc, yPos, width, rowHeight, scopeSelected, scopeTotal);
        rowCoordinates.add({ "label" => "", "y0" => yPos, "y1" => yPos + rowHeight,
                             "toggleAll" => true, "modeToggle" => false, "toggleX" => 0 });
        yPos += rowHeight;

        // Category or brand rows depending on mode
        var labels = appState.filterOrderBrandFirst
            ? FoodDatabase.getAllBrands(allItems)
            : FoodDatabase.getSubcategories(allItems);
        ...
        for (var i = 0; i < labels.size(); i++) {
            ...
            var selectedCount = appState.countSelected(scopeItems);
            var totalCount = scopeItems.size();

            drawRow(dc, yPos, width, rowHeight, label, selectedCount, totalCount, toggleX);
            rowCoordinates.add({ "label" => label, "y0" => yPos, "y1" => yPos + rowHeight,
                                 "toggleAll" => false, "modeToggle" => false, "toggleX" => toggleX });
            yPos += rowHeight;
        }
    }

    //! Dedicated mode-toggle row: gray background, centered text showing current order.
    private function drawModeToggleRow(dc as Dc, y as Lang.Number, width as Lang.Number,
                                       rowHeight as Lang.Number) as Void {
        var midY = y + rowHeight / 2;

        dc.setColor(Graphics.COLOR_DK_GRAY, Graphics.COLOR_BLACK);
        dc.fillRectangle(0, y, width, rowHeight);

        var modeText = appState.filterOrderBrandFirst
            ? "Marque > Categorie"
            : "Categorie > Marque";
        dc.setColor(Graphics.COLOR_WHITE, Graphics.COLOR_TRANSPARENT);
        dc.drawText(width / 2, midY, Graphics.FONT_XTINY, modeText,
    ...
    function subcategoryAtY(y as Lang.Number) as Lang.String? {
        ...
        if (isMode instanceof Lang.Boolean && isMode) { return null; }
